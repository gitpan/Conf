...
my $conf=new Conf(new Conf::String(\$string));
...
my $nstring=$string;
my $nconf=new Conf(new Conf::String(\$nstring));

ok($conf->get("test") eq "HI=Yes", "reuse commited Conf::String from \$string -> test=HI=Yes");
ok($conf->get("test1") eq "NO!", "reuse commited Conf::String from \$string -> test1=NO!");
ok($conf->get("test2") eq "%joep%", "reuse commited Conf::String from \$string -> test2=%joep%");
ok($conf->get("test3") eq "ok\n%Hello", "reuse commited Conf::String from \$string -> test3=ok");
ok($conf->get("oesterhol") eq "account","reuse commited Conf::String from \$string -> oesterhol=account");

### Test empty conf

my $estring="";
my $econf=new Conf(new Conf::String(\$estring));
...
my $ostring=$estring;
my $oconf=new Conf(new Conf::String(\$ostring));
